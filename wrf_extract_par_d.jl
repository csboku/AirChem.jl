@everywhere using NCDatasets,YAML,Proj,Dates,CFTime

@everywhere wrf_config = YAML.load_file("config.yaml")

@everywhere input_dir = wrf_config["input"]["folder"]
@everywhere input_files = readdir(input_dir,join=true)
@everywhere input_f = readdir(input_dir)

@everywhere output_dir = wrf_config["output"]["folder"]
@everywhere output_pattern = wrf_config["output"]["pattern"]


@everywhere wrf_vars_3d = wrf_config["variables_3d"]
@everywhere wrf_vars_2d = wrf_config["variables_2d"]
...
@sync  @distributed for f in input_files
    process_file(f)
end
