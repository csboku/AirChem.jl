...
function split_vector(v, n)
    ...
    for i in 1:n
        ...
        push!(chunks, v[start_idx:end_idx])
        start_idx = end_idx + 1
    end
...
function process_file_chunks(fname::String,outdir::String,outpattern::String,var3d::Vector{String},var2d::Vector{String})
    try
        log_message("Processing: $fname")
        ds = Dataset(fname)
        wrf_lat = ds["XLAT"][1,:,1]
        ...
        ## Get chunks
        time_chunks = split_vector(collect(1:length(ds[:XTIME])),Threads.nthreads())
        ...
        for var in var3d
            ...
            log_message(var)
            Threads.@threads for chunk in time_chunks
                wrf_ds_out[var][:,:,chunk] = ds[var][:,:,1,chunk]
            end
        end
...
export log_message,split_vector,process_file,process_file_chunks
